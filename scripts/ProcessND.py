...
if __name__ == "__main__":
    ...
    if any(x in stages for x in ["gen", "genie", "generator"]):
        ...
        if args.sam_name is not None:
            ...
            copylines.append( "generate_sam_json ${GHEP_FILE} ${RUN} ${NSPILL}
                    \"generated\" %s %1.2f %s %s %1.1f %d %s %s %s %s %s %s %s\n" %
                    (args.sam_name, args.oa, args.geometry, args.topvol, hc,
                        fluxid, args.data_stream, args.file_format,
                        args.application_family, args.application_name,
                        args.application_version, args.campaign, args.requestid) )
    ...
    if any(x in stages for x in ["g4", "geant4", "edepsim", "edep-sim"]):
        ...
        if args.sam_name is not None:
            copylines.append( "generate_sam_json ${EDEP_FILE} ${RUN} ${NSPILL}
                    \"simulated\" %s %1.2f %s %s %1.1f %d %s %s %s %s %s %s
                    %s\n" % (args.sam_name, args.oa, args.geometry, args.topvol,
                    hc, fluxid, args.data_stream, args.file_format,
                    args.application_family, args.application_name,
                    args.application_version, args.campaign, args.requestid) )
